...
if ( class_exists( "LD_History" ) == false ) {
	class LD_History extends LD_Mssql {
		...
		{
			global $tpl, $ODBC;                            
            $ODBC_Q = $ODBC->query("SELECT count(1) as total FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE login = '". $_SESSION['Login'] ."' AND type='common'");
            $ODBC_R = mssql_fetch_object($ODBC_Q);
            $tpl->set("TOTAL_BUYS",(int)$ODBC_R->total);
            
            $ODBC_Q = $ODBC->query("SELECT count(1) as total FROM [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits] WHERE login = '". $_SESSION['Login'] ."'");
            $ODBC_R = mssql_fetch_object($ODBC_Q);
            $tpl->set("TOTAL_BUYSKITS",(int)$ODBC_R->total);
			
			$ODBC_Q = $ODBC->query("SELECT count(1) as total FROM [". DB_SHOP_SQL ."].[dbo].[LogsPayments] WHERE login = '". $_SESSION['Login'] ."'");
			$ODBC_R = mssql_fetch_object($ODBC_Q);
        ...
        public function GetNameOptionJH($hexVal, $typeItemJH)
        {
            ...
            {
                $findHarmonyOptionNameQ = $ODBC->query("SELECT NM,prefx".substr($hexVal,1)." as lvl FROM [". DB_SHOP_SQL ."].[dbo].[ItemsJewelOfHarmony] WHERE TP='{$typeItemJH}'");
        ...
        public function GetNameOptionRefine($refine, $typeItemRF)
        {
            ...
            {
                $findRefineOptionNameQ = $ODBC->query("SELECT prefx1,prefx2 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsRefinery] WHERE ID={$typeItemRF}");
        ...
        public function GetNameOptionsSocketItem($socketsNumbers)
        {
            ...
            {                                 
                ...
                $SelectOptionsSocketQ = $ODBC->query("SELECT ST, NM, S1 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSocket] WHERE ID = {$socketNum}");
            ...
            else
            {
                ...
                $SelectOptionsSocketQ = $ODBC->query("SELECT ST, NM, S2 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSocket] WHERE ID = {$socketNum}");
                $SelectOptionsSocket = mssql_fetch_object($SelectOptionsSocketQ);
            ...
            else
            {
                ...
                $SelectOptionsSocketQ = $ODBC->query("SELECT ST, NM, S3 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSocket] WHERE ID = {$socketNum}");
                $SelectOptionsSocket = mssql_fetch_object($SelectOptionsSocketQ);
            ...
            else
            {
                ...
                $SelectOptionsSocketQ = $ODBC->query("SELECT ST, NM, S4 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSocket] WHERE ID = {$socketNum}");
                $SelectOptionsSocket = mssql_fetch_object($SelectOptionsSocketQ);
            ...
            else
            {
                ...
                $SelectOptionsSocketQ = $ODBC->query("SELECT ST, NM, S5 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSocket] WHERE ID = {$socketNum}");
                $SelectOptionsSocket = mssql_fetch_object($SelectOptionsSocketQ);
        ...
        {
            global $ODBC;
            $FindSoldsQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE login = '". $_SESSION['Login'] ."' AND type='common' ORDER BY number DESC");
            while($FindSolds = mssql_fetch_object($FindSoldsQuery))
            {    
                $IDI++;
                $FindItemDetailsQuery = $ODBC->query("SELECT NAME,EXE,photoItem,photoItemAnc,JH,RF,SetItem1,SetItem2 FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '". $FindSolds->itemNumber ."'");
        ...
        {         
            global $ODBC;  
            $findKitsSoldsQ = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits] WHERE login = '". $_SESSION['Login'] ."' ORDER BY Number DESC"); 
            ...
            {                                               //var_dump($findKitsSolds);
                ...
                {                           //var_dump($FindSoldsDetails);
                    ...
                    $IDI++;                                                       
                    $FindItemDetailsQuery = $ODBC->query("SELECT NAME,EXE,photoItem,photoItemAnc,JH,RF,SetItem1,SetItem2 FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '". $FindSoldsDetails->itemId ."'");
                    $FindItemDetails = mssql_fetch_object($FindItemDetailsQuery); 
                    $this->GetNameOptions($FindItemDetails->EXE);                 
                    $this->GetNameOptionJH($FindSolds->jh, $FindItemDetails->JH);
                    $this->GetNameOptionRefine($FindSolds->refine, $FindItemDetails->RF);
		...
		{
            global $ODBC;
			$ODBC_Q = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[LogsPayments] WHERE login='". $_SESSION['Login'] ."' ORDER BY id DESC");
