			...
			for($i = 0; $i < 120; $i++)
			{
				if($this->Vault_Slots[$i]['Hex'] <> str_pad("", DIVISOR, "F") && $this->Vault_Slots[$i]['Hex'] <> str_pad("", DIVISOR, "0") && $this->Vault_Slots[$i]['Hex'] <> NULL)
				{
					if(SYSTEM_ITEMS == "NEW")
					{
						...
						$FindItemDetailsQuery = $ODBC->query("SELECT Number, X, Y, NAME, DUR FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE ID = {$Index} AND TP = {$Categorie}");
					...
					else
					{
						...
						$FindItemDetailsQuery = $ODBC->query("SELECT Number, X, Y, NAME, DUR FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE ID = {$ID['index']} AND TP = ". (int)($ID['section']+$UniqueIntValue) ."");
						$FindItemDetails = mssql_fetch_object($FindItemDetailsQuery);
					}
