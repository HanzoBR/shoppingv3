...
if ( class_exists( "LD_Recover_Lost_Item" ) == false ) {
    ...
    public $BuyID, $ProductID, $ProductSerial, $Item_Level, $Item_Option, $Item_Ancient, $Item_Skill, $Item_Luck, $Item_OpExc_1, $Item_OpExc_2, $Item_OpExc_3, $Item_OpExc_4, $Item_OpExc_5, $Item_OpExc_6, $Item_JH;	
		...
		{
            global $ODBC;
			$ODBC_Q = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE number=". (int)$this->BuyID ." AND login = '". $_SESSION['Login'] ."'"); 
        ...
        {
            global $ODBC;
            $ODBC_Q = $ODBC->query("SELECT ID, TP, X, Y, DUR, CATEGORIA FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number='".$this->ProductID."'");
            $ODBC_R = mssql_fetch_object($ODBC_Q);
            $this->ID = $ODBC_R->ID;
            $this->TP = $ODBC_R->TP;         
            $this->X = $ODBC_R->X;
		...
		{
            global $ODBC;
			$ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[LogSolds] SET recovery = recovery + 1 WHERE number = ". (int) $this->BuyID);
		}
	}
}
