...
if ( class_exists( "LD_FinishBuy" ) == false ) {
    class LD_FinishBuy extends LD_Mssql {
     // public $End_Price, $Price, $Price_Level, $Price_Adicional, $Price_Skill, $Price_Luck, $Price_Ancient, $Price_OpExc, $ProductID, $Item_Level, $Item_Option, $Item_Ancient, $Item_Skill, $Item_Luck, $Item_OpExc_1, $Item_OpExc_2, $Item_OpExc_3, $Item_OpExc_4, $Item_OpExc_5, $Item_OpExc_6;
        ...
        {
            global $ODBC;
            $ODBC_Q = $ODBC->query("SELECT NAME, ID, CATEGORIA, TP, X, Y, DUR, LVL, RF, JH, Socket, ANC, SetItem1, SetItem2, price, priceLevel, priceOption, priceSkill, priceLuck, priceAncient, priceOptExc, priceJh, priceRefine, priceSocket, insertShop, maxOptExcSel FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number='".$this->ProductID."'");
        ...
        private function WriteLog()
        {
            global $LD_Items;
            $ODBC = $this;
            if($this->buyKit == false) $ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[Items] SET solds=solds+1 WHERE Number='".$this->ProductID."'");
            $ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[LogSolds] ([login], [itemNumber], [serial], [level], [option], [luck], [skill], [ancient], [excop1], [excop2], [excop3], [excop4], [excop5], [excop6], [jh], [refine], [socket1], [socket2], [socket3], [socket4], [socket5], [socket1_int], [socket2_int], [socket3_int], [socket4_int], [socket5_int], [price], [data], [recovery], [type]) VALUES 
