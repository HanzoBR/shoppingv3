...
if ( class_exists( "LD_FinishBuyKits" ) == false ) {
	class LD_FinishBuyKits extends LD_Mssql {
		...
		{ 
            ...
            $searchKitQ = $ODBC->query("SELECT priceFix FROM [". DB_SHOP_SQL ."].[dbo].[Kits] WHERE active = 1 AND Number = ". $_GET['ProductID'] ); 
            ...
            $searchItensKitQ = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE kitNumber = ". $_GET['ProductID'] );
            ...
            $ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[Kits] SET solds=solds+1 WHERE Number=".$_GET['ProductID']);
            $searchLastSoldNumberQ = $ODBC->query("SELECT max(Number) as Numb FROM [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits]");
            $searchLastSoldNumber = mssql_fetch_object($searchLastSoldNumberQ);
            $searchLastSoldNumber->Numb = (int)$searchLastSoldNumber->Numb+1;
            
            $ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits] (login,kitNumber,price,data) VALUES ('{$_SESSION['Login']}', {$_GET['ProductID']}, {$searchKit->priceFix}, '". time() ."')");   
            ...
            while($searchItensKit = mssql_fetch_object($searchItensKitQ))
            {
                ...
                $LD_FinishBuy = new LD_FinishBuy($searchItensKit->itemNumber,
                                                ...
                                                true);
                $ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[LogSoldsKitsDetails] (NumberSoldKit,login,itemId,itemSerial) VALUES ({$searchLastSoldNumber->Numb}, '{$_SESSION['Login']}', '{$searchItensKit->itemNumber}', '{$LD_Items->Item_Serial}')");   
