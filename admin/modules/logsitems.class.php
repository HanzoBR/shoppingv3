...
if ( class_exists( "LogsItems" ) == false ) {
	class LogsItems {
		...
		{
			global $tpl, $ODBC;
			$findTotalBuysQuery = $ODBC->query("SELECT count(1) as countSolds FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds]");
        ...
        {
            global $ODBC;
            $ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE type='common' AND number = ". (int) $_GET['id'] );
        ...
        private function Search_Buys()
        {
            ...
            $FindSoldsQuery = $ODBC->query("SELECT TOP ". $lasts ." * FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE type='common' {$query_p} ORDER BY number DESC");
            while($FindSolds = mssql_fetch_object($FindSoldsQuery))
            {    
                $IDI++;
                $FindItemDetailsQuery = $ODBC->query("SELECT NAME,EXE,photoItem,photoItemAnc,JH,RF FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '". $FindSolds->itemNumber ."'");
