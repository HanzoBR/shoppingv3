...
if ( class_exists( "Coupons" ) == false ) {
	class Coupons {
        ...
        {      
            ...
            $findCouponExistQuery = $ODBC->query("SELECT couponCode FROM [". DB_SHOP_SQL ."].[dbo].[CouponCodes] WHERE couponCode='{$couponCode}'");
            ...
            if($ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[CouponCodes] ([couponCode], [percent], [active], [dateBegin], [dateEnd]) VALUES ('{$couponCode}', {$percent}, {$active}, '{$dateBegin}', '{$dateEnd}');") == true)
        ...
        {
            global $ODBC;
            $memoryCouponsQuery = $ODBC->query("SELECT id, couponCode FROM [". DB_SHOP_SQL ."].[dbo].[CouponCodes] ORDER BY couponCode");
        ...
        {    
            ...
            $findDetailsCouponQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[CouponCodes] WHERE id = ". $_GET['number']);
        ...
        private function loadAlterCouponDb()
        {
            ...
            if($ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[CouponCodes] SET 
        ...
        private function loadRemoveCouponDb()
        {
            global $ODBC;
            if(is_numeric($_GET['number']) == false) exit("<ul><li>Erro, N&uacute;mero inv&aacute;lido.</li></ul>");
            if($ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[CouponCodes] WHERE id=". $_GET['number']) == true)
