...
if ( class_exists( "products" ) == false ) {
	class products
    {
        ...
        {
            global $ODBC;
            $memoryItemsQuery = $ODBC->query("SELECT Number, NAME, insertShop FROM [". DB_SHOP_SQL ."].[dbo].[Items] ORDER BY NAME");
        ...
        {
            global $ODBC;
            $memoryKitsQuery = $ODBC->query("SELECT Number, kitName FROM [". DB_SHOP_SQL ."].[dbo].[Kits] ORDER BY kitName");
        ...
        private function loadFormAlterProductFrom($number)
        {
            ...
            $findDetailsItemQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '{$number}'");    
            $findDetailsItem = mssql_fetch_object($findDetailsItemQuery);
            
            $findCategoriesQuery = $ODBC->query("SELECT Nombre FROM [". DB_SHOP_SQL ."].[dbo].[ItemsCategorias]");    
            ...
            $findDetailsAncQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[ItemsSetItems] ORDER BY Nombre");
        ...
        {   
            ...
            */
            $ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[Items] SET 
                            NAME = '{$this->name}',
                            TP = {$this->tp},
                            ID = {$this->id}, 
        ...
        private function loadFormIncludeProductDb()
        {
            ...
            $searchProductQ = $ODBC->query("SELECT 1 as Result FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '{$this->Number}'");
            ...
            $ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[Items] 
                    ([Number],
                    [TP],
                    [ID],
                    [NAME],
        ...
        {
            global $ODBC;
            if($ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number='{$_GET['number']}'") == false)
        ...
        private function loadIncludeKits()
        {
            ...
            $findLastNumberQuery = $ODBC->query("SELECT Number FROM [". DB_SHOP_SQL ."].[dbo].[Kits] ORDER BY Number DESC");
            $findLastNumber = mssql_fetch_object($findLastNumberQuery);
            $this->lastKitNumber = (int)$findLastNumber->Number + 1;
            
            if($ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[Kits] ([Number], [kitName], [priceFix], [solds], [active]) VALUES ({$this->lastKitNumber}, '{$_POST['kitName']}', {$priceFix}, {$solds}, {$active});") == true)
        ...
        private function loadFormAlterKitFrom($number)
        {
            ...
            {
                $findKitDetailsQuery = $ODBC->query("SELECT kitName FROM [". DB_SHOP_SQL ."].[dbo].[Kits] WHERE Number = ". $number);
                ...
                switch($_GET['action'])
                {
                    ...
                    case "includeItemForm":
                        $checkItemIncludeQuery = $ODBC->query("SELECT itemNumber FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE kitNumber = {$number} AND itemNumber='{$_GET['itemNumber']}'");
                        ...
                        $findItemDetailsQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number = '{$_GET['itemNumber']}'");
                        ...
                        {
                            $selectTagJh .= "<option value='00'>Nenhuma</option>";
                            $SelectOptionsJhQ = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[ItemsJewelOfHarmony] WHERE TP = '{$findItemDetails->JH}' ORDER BY [Number]");
                        ...
                        {
                            $SelectOptionRefineQ = $ODBC->query("SELECT prefx1, prefx2 FROM [". DB_SHOP_SQL ."].[dbo].[ItemsRefinery] WHERE ID={$findItemDetails->RF}");
                    ...
                    case "includeItemDb":
                        ...
                        if($ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] 
                    ...
                    case "alterItemSelect":
                        ...
                        $findItensInKitQuery = $ODBC->query("SELECT itemNumber FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE kitNumber = ". $number);
                        while($findItensInKit = mssql_fetch_object($findItensInKitQuery))
                        {
                                $findItemKitDetailsQuery = $ODBC->query("SELECT NAME FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number='{$findItensInKit->itemNumber}'");
                        ...
                        break;
                    case "alterItemForm":
                        $checkItemDetailsKitQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE kitNumber = {$number} AND itemNumber='{$_GET['itemNumber']}'");
                    ...
                    case "alterItemDb":
                        ...
                        if($ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] SET              
                                        [fixLVL] = {$fixLevel},
                                        [fixOP] = {$fixOP},
                                        [fixLuck] = {$fixLuck},
                                        [fixSkill] = {$fixSkill},
                    ...
                    case "removeItemDb":
                        ...
                        if($ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE [kitNumber] = {$number} AND [itemNumber] = '{$itemNumber}'") == false) 
                    ...
                    case "activeDesactiveSolds":
                        $findKitDetailsQuery = $ODBC->query("SELECT active FROM [". DB_SHOP_SQL ."].[dbo].[Kits] WHERE Number = ". $number);
                        $findKitDetails = mssql_fetch_object($findKitDetailsQuery);
                        if($findKitDetails->active == 0)
                        {
                            $ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[Kits] SET active = 1 WHERE Number = ". $number);
                            echo "<ul><li>As vendas desse kits foram <strong>ativadas</strong> nesse momento.</ul></li>";   
                        }
                        else
                        {
                            $ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[Kits] SET active = 0 WHERE Number = ". $number);
        ...
        private function loadFormSelectRemoveKitDb()
        {
            ...
            if($ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[KitsItemsDetails] WHERE kitNumber = ". $_GET['number']) == true) echo "ok.</li></ul>"; else echo "erro.</li></ul>";
            echo "<ul><li>Deletando kit:";
            if($ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[Kits] WHERE Number = ". $_GET['number']) == true) echo "ok.</li></ul>"; else echo "erro.</li></ul>";  
        }
	}
}
