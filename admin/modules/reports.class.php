...
if ( class_exists( "Reports" ) == false ) {
	class Reports {
		...
		private function GenerateReports()
		{
            ...
            $this->findPaymentsQuery = $ODBC->query("SELECT price FROM [". DB_SHOP_SQL ."].[dbo].[LogsReports] WHERE dateInclude > {$timeOneBegin} AND dateInclude < {$timeOneEnd}");
            while($this->findPayments = mssql_fetch_object($this->findPaymentsQuery))
                 $this->totalPaymentsValueOne += $this->findPayments->price;
            
            $this->findPaymentsQuery = $ODBC->query("SELECT price FROM [". DB_SHOP_SQL ."].[dbo].[LogsReports] WHERE dateInclude > {$timeTwoBegin} AND dateInclude < {$timeTwoEnd}");
            while($this->findPayments = mssql_fetch_object($this->findPaymentsQuery))
                 $this->totalPaymentsValueTwo += $this->findPayments->price;
                 
            $this->findSoldsItemsOneQuery = $ODBC->query("SELECT count(1) as totalSolds FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE data > '{$timeOneBegin}' AND data < '{$timeOneEnd}'");
            $this->findSoldsItemsOne = mssql_fetch_object($this->findSoldsItemsOneQuery);
            $this->findSoldsItemsTwoQuery = $ODBC->query("SELECT count(1) as totalSolds FROM [". DB_SHOP_SQL ."].[dbo].[LogSolds] WHERE data > '{$timeTwoBegin}' AND data < '{$timeTwoEnd}'");
            $this->findSoldsItemsTwo = mssql_fetch_object($this->findSoldsItemsTwoQuery);
            
            $this->findSoldsKitsOneQuery = $ODBC->query("SELECT count(1) as totalSolds FROM [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits] WHERE data > '{$timeOneBegin}' AND data < '{$timeOneEnd}'");
            $this->findSoldsKitsOne = mssql_fetch_object($this->findSoldsKitsOneQuery);
            $this->findSoldsKitsTwoQuery = $ODBC->query("SELECT count(1) as totalSolds FROM [". DB_SHOP_SQL ."].[dbo].[LogSoldsKits] WHERE data > '{$timeTwoBegin}' AND data < '{$timeTwoEnd}'");
