...
if ( class_exists( "Payments" ) == false ) {
	class Payments extends LD_Mssql {
		...
		{
			global $tpl, $ODBC;
			$findPaymentsQuery = $ODBC->query("SELECT count(1) as countResult FROM [". DB_SHOP_SQL ."].[dbo].[LogsPayments] WHERE status = ". $status);
		...
		private function Write_Confirmation()
		{
			...
			switch($status)
			{
				case 1:   
				    ...
				    $ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[LogsReports] WHERE number = {$Id}");
                break;
				case 2:    
					$ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[LogsPayments] SET [comentario_adm] = '". base64_encode($comentario_adm) ."', [status] = 2 WHERE [id] = ". $Id);
					$this->query("UPDATE ". GOLDTABLE ." SET ". GOLDCOLUMN ." = ". GOLDCOLUMN ." + ". $golds ." WHERE ". GOLDMEMBIDENT ." = '". $login ."'");
				    $ODBC->query("INSERT INTO [". DB_SHOP_SQL ."].[dbo].[LogsReports] ([number], [login], [price], [dateInclude]) VALUES ({$Id},'{$login}','{$valor}',". time() .");");
                break;  
				case 3:  
					$ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[LogsPayments] SET [comentario_adm] = '". base64_encode($comentario_adm) ."', [status] = 3 WHERE [id] = ". $Id);
				    $ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[LogsReports] WHERE number = {$Id}");
                break;
				case 4:       
					$ODBC->query("UPDATE [". DB_SHOP_SQL ."].[dbo].[LogsPayments] SET [comentario_adm] = '". base64_encode($comentario_adm) ."', [status] = 4 WHERE [id] = ". $Id);
					$this->query("UPDATE ". GOLDTABLE ." SET ". GOLDCOLUMN ." = ". GOLDCOLUMN ." - ". $golds ." WHERE ". GOLDMEMBIDENT ." = '". $login ."'");
				    $ODBC->query("DELETE FROM [". DB_SHOP_SQL ."].[dbo].[LogsReports] WHERE number = {$Id}");
                break;
			}
		...
		{
			global $tpl, $ODBC;
			$findPaymentsQuery = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[LogsPayments] WHERE status = ". $status);
